...
var MessageModule = function (socket) {
  ...
  socket.on('message send', function (data) {
    ...
    var newMessage = MessageModel({
      ...
      channel: (data.channel === undefined ? 'general' : data.channel), // если канал не пришёл, пишем в general
      text: data.text,
      raw: data.text,
      type: (data.type === undefined ? 'text' : data.type) // если не пришёл тип, то думаем, что это текст
    });
